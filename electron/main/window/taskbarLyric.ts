...
import type {
  ...
  TrayWatcher,
  UiaWatcher,
} from "@splayer/smtc-lyric";

type TaskbarLyricNative = typeof import("@splayer/smtc-lyric");
...
interface PickedSpace {
  rect: JsTaskbarLayout["space"]["left"];
  anchor: AnchorSide;
}
...
const pickSpace = (layout: JsTaskbarLayout): PickedSpace | null => {
  const position = (store.get("taskbarLyric.position") as string) ?? "auto";
...
const applyLayout = (layout: JsTaskbarLayout): void => {
  ...
  const leftMargin = (store.get("taskbarLyric.leftMargin") as number) ?? 0;
  const rightMargin = (store.get("taskbarLyric.rightMargin") as number) ?? 0;
  ...
  const autoMaxWidth = (store.get("taskbarLyric.autoMaxWidth") as boolean) ?? true;
  const maxWidth = (store.get("taskbarLyric.maxWidth") as number) ?? 400;
...
export const createTaskbarLyricWindow = (): BrowserWindow | null => {
  ...
  if (!nativeModule) {
    nativeModule = loadNativeModule<TaskbarLyricNative>("smtc-lyric.node", "smtc-lyric");
    if (!nativeModule) {
