...
/// 读取进程可执行文件完整路径（小写），失败返回 None
pub fn process_image_name(pid: u32) -> Option<String> {
    // SAFETY: OpenProcess/QueryFullProcessImageNameW 对受限进程失败时返回 Err/0，句柄成对关闭
    unsafe {
        ...
        Some(String::from_utf16_lossy(&buf[..size as usize]).to_ascii_lowercase())
    }
}
...
unsafe fn is_explorer_window(hwnd: HWND) -> bool {
    ...
    pid != 0 && process_image_name(pid).is_some_and(|p| p.ends_with("\\explorer.exe"))
}
